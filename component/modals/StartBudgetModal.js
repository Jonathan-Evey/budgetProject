...
const StartBudgetModal = props => {
  ...
  const [addAdditionalIncomeError, setAddAdditionalIncomeError] =
    useState(false);
  ...
  const saveBudgetDataAdditionalIncome = async (
    userID,
    monthData,
    yearData,
  ) => {
    await setDoc(doc(db, 'users', userID), {
      mainBudget: parseInt(mainBudget),
      additionalIncome: {
        amount: parseInt(additionalIncome),
        month: monthData,
        year: yearData,
      },
    })
    ...
    <View style={styles.container}>
      <View style={styles.budgetFormContainer}>
        ...
        <View>
          ...
          <TextInput
            value={mainBudget}
            onChangeText={text => setMainBudget(text)}
          />
          {addMainBudgetError ? (
            <Text>Please add your budget amount</Text>
          ) : null}
        </View>
        {addAdditionalIncome ? (
          <View>
            <TouchableOpacity
              onPress={() => setAddAdditionalIncome(!addAdditionalIncome)}>
              <Text>X</Text>
            </TouchableOpacity>
            ...
            />
            {addAdditionalIncomeError ? (
              <Text>Please include the amount to add</Text>
            ) : null}
          </View>
        ) : (
          <TouchableOpacity
