...
const AddIncomeModal = (props) => {
  const [ifAdditionalIncome, setIfAdditionalIncome] = useState(`${props.userData.additionalIncome.amount}`)
  const [additionalIncome, setAdditionalIncome] = useState('')
  const [noAdditionalIncomeAddedError, setNoAdditionalIncomeAddedError] = useState(false)
  ...
  const saveAdditionalIncome = async (userID, amountData, monthData, yearData) => {
    ...
    <View style={styles.container}>
      <View style={styles.incomeFormContainer}>
        ...
        <View>
          ...
          {props.isAdditionalIncome ? <TextInput keyboardType="numeric" value={ifAdditionalIncome} onChangeText={text => setIfAdditionalIncome(text.replace(/[^0-9]/g, ''))}/> :
          <TextInput keyboardType="numeric" value={additionalIncome} onChangeText={text => setAdditionalIncome(text.replace(/[^0-9]/g, ''))}/>}
          
          {noAdditionalIncomeAddedError ? <Text>Please add the amount of extra income for this month</Text> : null}
        </View>
